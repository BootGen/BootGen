...
import { delay, getJsonLength, toCamelCase } from './Helper';
...
export function hideSnackbar(viewModel: ViewModel){
    viewModel.snackbar.visible = false;
}
...
export function validateAndGenerate (viewModel: ViewModel) {
    ...
    if(!result.error) {
        prettyPrint(viewModel.activeProject.json);
        const jsonLength = getJsonLength(viewModel.activeProject.json);
        if(jsonLength > 2000) {
        ...
        viewModel.jsonErrors.push({line: result.line, color: 'rgba(255, 0, 0, 0.3)'});
        setSnackbar(viewModel, 'orange darken-2', result.message, -1);
    }
}
