<template>
  <v-container fluid class="pa-0 ma-0" v-if="commandStore.command(CommandType.OpenNewProjectDialog) !== null">
        ...
        <div class="d-flex align-center">
          ...
          <ValidationObserver>
            <ValidationProvider v-slot="{ errors }" name="Project name" rules="required">
              <v-text-field v-model="projectName" placeholder="Name your project" type="text" :error-messages="errors" required @input="changeName" :disabled="false"></v-text-field>
            </ValidationProvider>
          </ValidationObserver>
          <v-btn class="mr-0 ml-3" color="primary" small @click="commandStore.do(CommandType.OpenNewProjectDialog)" v-if="$store.state.auth.jwt" :disabled="false">{{ commandStore.command(CommandType.OpenNewProjectDialog).text }}</v-btn>
        </div>
